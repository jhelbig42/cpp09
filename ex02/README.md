...
- compare input elements n and n + 1 to determine which element has the smaller value (pushed to _pend vector) and which the bigger (pushed to _main vector).
    - _main vector elements are added as pairs, storing their original index to later determine their corresponding _pend element. Thus keeping the original assigned pairs
    - if there is an odd amount of input values: push the remaining, not pairable, value to _pend
- recursively sorting _main vector using merge sort
- reordering _pend vector, so the original pairs are kept
- determine insertion order using Jacobsthal sequence
- create _result vector. $b_0$, which is _pend[0] becomes the first element of _result, followed by all the elements of _main (representing $a_n$)
- create mainPos vector, which contains the current index within _result of the elements $a_n$ within _main
- insert _pend element $b_n$ in insertion order into _result using binary insertion sort
